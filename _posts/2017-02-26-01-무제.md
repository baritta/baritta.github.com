...
수식을 사용하려면 `enable_mathjax` 를 켜줘야함. _layouts에 있는 default.html, page.html, post.html 파일에 아래 라인을 추가한다. (이 부분 때문에 엄청 시간을 낭비했다. 이건 테마 마다 다른듯 하다. 세 파일 중 어떤게 상관있는건지 잘 모르겠다.아 또 안되. 으아. 악.)
```
